<template>
  <div class="modal" @click.prevent="closeModalOutSideDialog">
    ...
    <teleport to="body">
      <ModalConfirm v-show="modalConfirm.name === 'category-document-delete' && modalConfirm.open === true" :text="setConfirmMessage" />
    </teleport>
  </div>
</template>
...
export default {
  ...
  setup(props, {emit}) {
    ...
    const createCategory = async () => {
      ...
      try {
        await store.dispatch('category/createCategoryDocuments', category.value)
        category.value = ''
      } catch (e) {
    ...
    const setConfirmMessage = computed(() => {
      return `Удалить категорию: - ${elementName.value} - и все документы в ней`
    })
    ...
    watch(modalConfirm, async (val) => {
      if (modalConfirm.value.name === 'category-document-delete') {
    ...
    const removeCategory = async () => {
      await store.dispatch('category/deleteCategory', element.value)
