  ...
  <div class="modal" @click.prevent="closeModalOutSideDialog">
    <div class="container" @click.prevent>
      <div class="modal__dialog">
        ...
        <div :class="['counter', isError ? 'error' : '']">
          {{textarea.length}} / 180
        </div>
        <textarea class="modal__textarea" v-model="textarea" @input="changeTextarea" maxlength="180">Описание</textarea>
      </div>
    </div>
  </div>
</template>
...
setup(props, {emit}) {
    ...
    const isError = computed(() => {
      return textarea.value.length >= 180
    })
