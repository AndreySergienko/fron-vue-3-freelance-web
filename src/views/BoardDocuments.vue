      ...
      <div class="panel">
        <div class="panel__inner">
          <div class="panel__control">
            <TheDocuments @render="renderDocument" @selectDocument="selectDocument" />
            ...
            <form class="form form-create-documents backdrop-filter-auto" enctype="multipart/form-data" @submit.prevent="onSubmit">
              ...
              <div class="form__buttons form__buttons-create">
                ...
                <div>
                  <input type="file" class="input-file" id="file" ref="file" @change="entryFileName"/>
                  <label for="file" class="label-file" v-if="!getFileName">
                    <svg width="22" height="24" viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M8.79473 1.50897C7.82164 0.535884 6.52796 0 5.15185 0C3.77573 0 2.48205 0.535884 1.50897 1.50897C0.535883 2.48205 0 3.77573 0 5.15185C0 6.52796 0.535883 7.82165 1.50897 8.79473L11.7151 19.0009C13.0349 20.3205 15.1815 20.3201 16.5009 19.0009C17.8203 17.6813 17.8203 15.5347 16.5009 14.2151L6.39561 4.1101C6.007 3.7215 5.37675 3.7215 4.98831 4.1101C4.59971 4.4987 4.59971 5.1288 4.98831 5.5174L15.0935 15.6226C15.6368 16.1659 15.6368 17.0503 15.0935 17.5936C14.5501 18.137 13.6659 18.137 13.1224 17.5936L2.91626 7.3876C2.31901 6.79035 1.99028 5.99639 1.99028 5.15201C1.99028 4.30747 2.31918 3.51351 2.91626 2.91643C4.14908 1.68361 6.15462 1.68361 7.38743 2.91643L18.0812 13.6102C19.9997 15.5287 19.9997 18.6503 18.0812 20.5688C17.1519 21.498 15.9163 22.0099 14.602 22.0099C13.2878 22.0099 12.0522 21.498 11.1229 20.5688L2.90084 12.3466C2.51223 11.958 1.88198 11.958 1.49354 12.3466C1.10494 12.7352 1.10494 13.3653 1.49354 13.7539L9.71556 21.9759C11.0209 23.2812 12.7562 24 14.602 24C16.4479 24 18.1834 23.2812 19.4885 21.9759C20.7936 20.6706 21.5126 18.9352 21.5126 17.0892C21.5126 15.2434 20.7938 13.5079 19.4885 12.2028L8.79473 1.50897Z" fill="#DDDDDD"/>
                    </svg>
                    Выберите файл
                  </label>
                  <label for="file" class="label-file btn-primary-edit" v-else>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M23.7285 4.26403L19.7361 0.271184C19.5625 0.0975128 19.327 0 19.0814 0C18.8359 0 18.6004 0.0975128 18.4269 0.271061L14.4338 4.26415C14.4333 4.26464 14.4328 4.26538 14.4322 4.26588L6.77336 11.9247C6.6066 12.0916 6.50983 12.3155 6.50267 12.5513L6.37738 16.6691C6.36961 16.9243 6.46761 17.1713 6.64807 17.3518C6.82199 17.5256 7.0575 17.6229 7.30264 17.6229C7.31202 17.6229 7.32153 17.6228 7.33091 17.6224L11.4487 17.4968C11.6843 17.4896 11.9083 17.3928 12.0751 17.2262L23.7285 5.57304C24.09 5.21175 24.09 4.62556 23.7285 4.26403ZM19.0811 7.60193L16.3976 4.91884L19.0814 2.23502L21.7647 4.9186L19.0811 7.60193ZM11.0254 15.6576L8.2579 15.7421L8.34208 12.9745L15.0884 6.22811L17.7719 8.91131L11.0254 15.6576Z" fill="#FFD699"/>
                      <path d="M16.3165 14.4532C15.8052 14.4532 15.3907 14.8675 15.3907 15.3789V22.1485H1.85151V8.60932H8.62112C9.13239 8.60932 9.54688 8.19495 9.54688 7.68357C9.54688 7.17218 9.13239 6.75781 8.62112 6.75781H0.925755C0.414491 6.75781 0 7.17218 0 7.68357V23.0742C0 23.5856 0.414491 23.9999 0.925755 23.9999H16.3164C16.8276 23.9999 17.2421 23.5856 17.2421 23.0742V15.3788C17.2422 14.8675 16.8278 14.4532 16.3165 14.4532Z" fill="#FFD699"/>
                    </svg>
              ...
              <div class="create__file" v-if="getFileName" >
                {{ getFileName }}
              </div>

              <div class="create__file" v-else>
                * Не используйте точки в названии файла
              </div>

              <hr class="create__line">

              <div :class="['form__buttons form__buttons-create', {'one-btn': isCreate || !getCurrentDocument}]">
                <button class="btn btn-important btn-important-delete" type="button" @click="removeDocument" v-if="!isCreate && !!getCurrentDocument">Удалить документ</button>
            ...
            <teleport to="body">
              <ModalConfirm v-show="modalConfirm.name === 'document-delete' && modalConfirm.open === true" :text="setConfirmMessage" />
              <ModalVariablesCategoryOnlyRead v-show="isModal === 'variables-category-read'" @change="entryCategory" products="documents"/>
          ...
          <div class="panel__info backdrop-filter-auto">
            <InfoBlock :errors="[tError, pError, textError, catError, flsError]" />
          </div>
...
import TheDocuments from "../components/boardDocuments/TheDocuments"
import InfoBlock from "../components/InfoBlock"
import {useDocumentsForm} from "../use/documents-form"
...
export default {
  setup() {
    ...
    watch(modalConfirm, (val) => {
      if (modalConfirm.value.name === 'document-delete') {
        if (modalConfirm.value.isResult !== true) return
        deleteDocumentOnClick()
      }
    })
    ...
    const getCurrentDocument = computed(() => {
      return currentDocument.value
    })

    const setConfirmMessage = computed(() => {
      return `Удалить документ:  ${currentDocument.value?.title}`
    })

    const deleteDocumentOnClick = async () => {
      await store.dispatch('document/deleteDocument', currentDocument.value.id)
    ...
    const openModalCategory = () => {
      store.commit('setModal', {
        open: true,
        name: 'variables-category-read'
      })
    }

    const selectDocument = (obj) => {
